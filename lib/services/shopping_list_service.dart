...
class ShoppingListService {
  ...
  static Future<List<Map<String, dynamic>>> addItem(
    ...
    final existingItemIndex =
        currentList.indexWhere((i) => i['id'] == item['id']);

    if (existingItemIndex >= 0) {
      // Update quantity if item exists
      currentList[existingItemIndex]['quantity'] =
          (currentList[existingItemIndex]['quantity'] as int) +
      ...
      if (!item.containsKey('inCart')) {
        item['inCart'] = false; // Set to true by default when added via QR scan
      }
  ...
  static Future<List<Map<String, dynamic>>> updateStatus(
      Map<String, dynamic> item) async {
    final List<Map<String, dynamic>> currentList = await getShoppingList();
    ...
    if (existingItemIndex >= 0) {
      // Just update inCart status to true (no quantity change)
      currentList[existingItemIndex]['inCart'] = true;
    } else {
      // Don't add new item if not found
      print("Item not found in list, skipping addition");
    }
  ...
  static Future<void> toggleItemInCart(String itemId) async {
    ...
    if (itemIndex >= 0) {
      ...
      currentList[itemIndex]['inCart'] =
          !(currentList[itemIndex]['inCart'] ?? false);
      await saveShoppingList(currentList);
    }
  }
